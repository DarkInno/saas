...
func (plugin *Plugin) Initialize(db *gorm.DB) error {
	if err := db.Callback().Query().Before("gorm:query").Register("gotenancy:query", plugin.addTenantCondition); err != nil {
		return err
	}
	if err := db.Callback().Query().Before("gorm:preload").Register("gotenancy:preload", plugin.guardPreloads); err != nil {
		return err
	}
	...
	if err := db.Callback().Row().Before("gorm:row").Register("gotenancy:row", plugin.addTenantCondition); err != nil {
		return err
	}
	if err := db.Callback().Raw().Before("gorm:raw").Register("gotenancy:raw", plugin.requireHostForRaw); err != nil {
		return err
	}
	return nil
}
