...
GoTenancy 是一个与 ORM 无关的 Go 工具包，用于实现以必需 `tenant_id` 边界为基础的共享数据库多租户。
...
- `saas/tenant`：租户生命周期状态机。
- `saas/plan`：套餐 Store、内存实现和 `database/sql` SQLStore。
- `saas/subscription`：订阅生命周期、续订、过期、宽限期处理、计费 hook、Store、内存实现和 `database/sql` SQLStore。
- `saas/quota`：配额检查、原子消耗、重置、内存实现和 `database/sql` SQLStore。
- `saas/feature`：套餐默认功能、租户级功能覆盖、内存实现和 `database/sql` SQLStore。
- `saas/onboarding`：涵盖租户、套餐、订阅、功能、配额、审计和通知服务的租户开通流程。
